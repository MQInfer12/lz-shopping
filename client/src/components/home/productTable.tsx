...
import { deleteProduct, toggleProduct } from "../../services/product";
...
const ProductTable = ({
  ...
  const { products, editProduct, removeProduct } = useData();
  ...
  const handleToggle = async (id: number) => {
    ...
    Swal.fire({
      title: "Petición correcta",
      text: `Se ${
        res.data.stock === 0 ? "desactivo" : "activo"
  ...
  const handleEdit = async (product: Product) => {
    ...
    const quantitySold =
      product?.clients?.reduce((ac, sale) => ac + sale.amount, 0) || 0;
        ...
        <tbody>
          {products.map((product, i) => {
            /* const stock =
              ...
              product.clients &&
              product.stock -
                product?.clients?.reduce((ac, sale) => ac + sale.amount, 0); */
            const stock = product.stock;
            return (
              <tr key={product.id}>
                <td>
                  <img src={product.photo} alt={`productPhoto${product.id}`} />
                </td>
                <td style={{ maxWidth: "15rem" }}>{product.name}</td>
                <td className="center">{product.price} Bs.</td>
                <td className="center">
                  {product.discount ? product.discount + " Bs." : "N/A"}
                </td>
                <td className="center">
                ...
                <td className="center padding">
                  {product.categories?.length} seleccionado(s).
                </td>
